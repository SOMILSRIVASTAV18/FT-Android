...
export default function App() {
              ...
              if (aiData && aiData.amount > 0) {
                ...
                if (snapshot.empty) {
                  const txData = {
                    ...
                    fromAccount: aiData.accountLast4 ? `${aiData.bankName || 'Bank'} (${aiData.accountLast4})` : null,
                    paymentMode: 'Account'
                  };
  ...
  const navItems = [
    ...
    { id: 'upi-scanner', label: 'Scan & Pay', icon: QrCode },
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'upi-scanner': return <UpiScanner profile={profile!} onPaymentComplete={() => setActiveTab('transactions')} />;
      case 'transactions': return <Transactions transactions={transactions} profile={profile!} family={family} onTabChange={setActiveTab} />;
  ...
  return (
    <div className={cn("h-screen h-[100dvh] w-full overflow-hidden flex flex-col md:flex-row bg-background selection:bg-primary/30", profile?.settings.darkMode && "dark")}>
      ...
      {/* Mobile Top Header */}
      <header className="md:hidden shrink-0 flex items-center justify-between p-4 pt-[calc(env(safe-area-inset-top)+1rem)] border-b bg-card/80 backdrop-blur-xl z-30 dark:bg-black/90">
      ...
      {/* Main Content Area */}
      <main className={cn(
        "flex-1 min-h-0 overflow-y-auto overscroll-contain p-4 md:p-10 pb-32 md:pb-10 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-primary/5 via-transparent to-transparent",
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 px-4 pb-6 pt-2 bg-gradient-to-t from-background via-background/90 to-transparent dark:from-black dark:via-black/90">
        <div className="relative glass rounded-[2.5rem] flex items-center justify-between px-2 h-20 shadow-2xl shadow-primary/20 border border-white/20 dark:bg-zinc-900/90 dark:border-white/10">
          {/* Left side items */}
          <div className="flex flex-1 justify-around">
            {[navItems[0], navItems[2]].map((item) => (
              <button
                ...
                className={cn(
                  "flex flex-col items-center justify-center w-14 h-14 rounded-2xl transition-all duration-200 relative active:scale-95",
                  activeTab === item.id && !isMobileMenuOpen ? "text-primary" : "text-muted-foreground"
                )}
              >
                <item.icon className={cn("h-6 w-6 transition-transform duration-200", activeTab === item.id && !isMobileMenuOpen && "scale-110")} />
          ...
          <div className="relative -top-5 flex flex-col items-center shrink-0">
            <button 
              onClick={() => {
                setActiveTab('upi-scanner');
                setIsMobileMenuOpen(false);
              }}
              className={cn(
                "w-14 h-14 rounded-full flex items-center justify-center shadow-2xl border-4 border-background active:scale-95 transition-all",
                activeTab === 'upi-scanner'
                  ? "bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-violet-500/50 ring-4 ring-primary/30 scale-105"
                  : "bg-gradient-to-r from-primary to-violet-600 text-primary-foreground shadow-primary/40 hover:scale-105"
              )}
              title="Scan UPI QR"
            >
              <QrCode className="h-7 w-7" />
            </button>
            <span className="text-[9px] font-black uppercase tracking-wider text-primary mt-1">Scan QR</span>
          </div>
          ...
          <div className="flex flex-1 justify-around">
            ...
            >
              <PieChart className={cn("h-6 w-6 transition-transform duration-200", activeTab === 'budgets' && !isMobileMenuOpen && "scale-110")} />
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden fixed inset-0 z-40 bg-background dark:bg-zinc-950 pt-20 pb-32 px-6 overflow-y-auto overscroll-contain touch-pan-y"
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {navItems.map((item) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center p-3 rounded-2xl transition-all duration-150 active:scale-95",
                    activeTab === item.id ? "bg-primary/15 text-primary border border-primary/20" : "bg-card/70 hover:bg-primary/5 border border-border/50 text-muted-foreground"
                  )}
                >
                  <div className={cn(
                    "w-10 h-10 rounded-xl flex items-center justify-center mb-1.5 shadow-xs",
