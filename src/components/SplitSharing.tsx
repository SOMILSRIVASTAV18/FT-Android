...
export function SplitSharing({ profile }: SplitSharingProps) {
  ...
  const handleJoinGroup = async (e: React.FormEvent) => {
    ...
    const path = 'splitGroups';
    try {
      const q = query(collection(db, path), where('code', '==', joinCode.trim().toUpperCase()));
      ...
      if (groupData.members && groupData.members.includes(profile.uid)) {
        toast.error('You are already a member of this group');
        return;
      }

      const updatedMembers = [...(groupData.members || []), profile.uid];
      const updatedMemberNames = {
        ...(groupData.memberNames || {}),
        [profile.uid]: profile?.displayName || profile?.email?.split('@')[0] || 'User'
      };
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const membersCount = selectedGroup.members?.length || 1;
      const splitAmount = amount / membersCount;
      const splits: { [uid: string]: number } = {};
      (selectedGroup.members || [profile.uid]).forEach(uid => {
      ...
      const expenseData = {
        ...
        payerId: profile.uid,
        payerName: profile?.displayName || profile?.email?.split('@')[0] || 'User',
        amount,
